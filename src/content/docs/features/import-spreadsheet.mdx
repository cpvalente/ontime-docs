...
description: Import Excel and spreadsheet rundowns into Ontime to create events, cues, notes, custom fields, and production timing.
---
...
2. You will provide a reference of the spreadsheet column names on import
3. Ontime creates a rundown entry for every row on the table (see exception in Timer type notes)
4. The times can be either an Excel time field or a short text entry as described [in the smart entry feature](/quick-tips/smart-time-entry) eg: `00:10:15` or `10m15s`
