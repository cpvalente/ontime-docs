...
- `Removed` __subtitle__ and __presenter__ fields in event in favour of fully customisable __custom fields__
...
- `Breaking` __Roll mode__ needs ordered events*
- `Breaking` restructure of all __APIs__

:::note[*Roll mode]
Previously you could have events in any order and __Roll Mode__ would figure out what should be playing.
With v3, we expect that the events are in the correct order as Ontime will look for the __first event with matching time__.

We promise we will make it worth it.
:::