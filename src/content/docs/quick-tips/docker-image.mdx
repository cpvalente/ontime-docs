...
* `"./ontime-external:/external/"` expose the internal container directorie.
...
You can bind a single host directory to `/external/` for Ontime to store all its files in, or
these three paths can be bound to individual host directories as in the sample docker-compose file:
