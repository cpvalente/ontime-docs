...
Namely: [Addded time](#addded-time), [Scheduled delays](#scheduled-delays) and [offsets](#offset).
...
__Positive__ -> offset indicates time delayed \
__Negative__ -> offset indicates time ahead
:::
...
:::tip[In combination with time-to-end timers]
Time to end timers - since they count to their finish time regardless of when they were started - always finish on time unless they are in overtime.

An offset will only be added if the event is in overtime or the user has added time.
