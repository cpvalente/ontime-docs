...
description: Use Ontime smart time entry to define event durations, start times, linked events, and schedule strategies in live rundowns.
---
...
You can achieve this by activating the Link start feature with the lock button next to the `Start time` field (_See note 1 in screenshot_). \
...
In your rundown, you can define this per event, by activating the lock next to the input field (_See illustration note 2 for locking end, note 3 for locking duration_).
